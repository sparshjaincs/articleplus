...
class Poetries(models.Model):
    STATUS_CHOICES = (
		('draft', 'Draft'),('published', 'Published'),)
    METHOD_CHOICES = (
		('content', 'content'),('design', 'design'),('editor','editor'))
    user_poetries = models.ForeignKey(User, related_name='user3', to_field='username', on_delete=models.CASCADE)
    ...
    category = models.ForeignKey(Categories,related_name = 'category3', to_field='category_name',on_delete=models.CASCADE,default=" ")
    ...
    liked = models.ManyToManyField(User,default=None,blank=True,related_name="likes_poetries")
    disliked = models.ManyToManyField(User,default=None,blank=True,related_name="dislikes_poetries")
...
class Poetries_comment(models.Model):
    post = models.ForeignKey(Poetries,related_name='poetris_comments',on_delete=models.CASCADE)
    ...
    class Meta:
        ordering = ('-created',)
    def children(self):
        return Poetries_comment.objects.filter(parent=self)
...
class titleview(models.Model):
    view = models.ForeignKey(Poetries,related_name="poetriesview",to_field='title',on_delete=models.CASCADE)
